...
namespace leveldb {
    ...
    Options SanitizeOptions(const std::string& dbname,
        ...
        if (result.info_log == NULL) {
            ...
            src.env->RenameFile(InfoLogFileName(dbname), OldInfoLogFileName(dbname)); // NOTE:htt,将原有的{dbname}/LOG文件 重命{dbname}/LOG.old
    ...
    Status DBImpl::Recover(VersionEdit* edit) { // NOTE:htt, 从CURRENT读取mainfest,获取该快照所有文件,在从WAL日志中恢复数据->memtable->sst文件
        mutex_.AssertHeld();
        ...
        s = versions_->Recover(); // NOTE:htt, 从${dbname}/CURRENT获取mainfest文件名,从mainfest读取VersionEdit列表恢复对应Version
        if (s.ok()) { // NOTE:htt, 从WAL日志中恢复数据
            ...
            for (size_t i = 0; i < logs.size(); i++) { // NOTE:htt, 从所有的WAL日志会数据
                s = RecoverLogFile(logs[i], edit, &max_sequence); // NOTE:htt, 从WAL日志中恢复数据到memtable,再将memtable生成sst文件
    ...
    Status DBImpl::RecoverLogFile(uint64_t log_number,
        ...
        while (reader.ReadRecord(&record, &scratch) && // NOTE:htt, 从WAL中读取一条完整日志;该record为一批<key,value>
            ...
            const SequenceNumber last_seq =
                ...
                WriteBatchInternal::Count(&batch) - 1; // NOTE:htt, 最后一条插入记录的seq
    ...
    Status DBImpl::WriteLevel0Table(MemTable* mem, VersionEdit* edit,
        ...
        {
            mutex_.Unlock();
            s = BuildTable(dbname_, env_, options_, table_cache_, iter, &meta);// NOTE:htt, 将iter中的数据内容写入到sst文件中(dbname为前缀),生成元信息到meta
            mutex_.Lock();
        }
        ...
        if (s.ok() && meta.file_size > 0) {
            ...
            if (base != NULL) {
                level = base->PickLevelForMemTableOutput(min_user_key, max_user_key); // NOTE:htt, 选择MemTable可以直接落入层,范围level [0,1,2]这三层文件,目标避免0->1的compact
            }
            edit->AddFile(level, meta.number, meta.file_size,
                    meta.smallest, meta.largest); // NOTE:htt, 添加保存sst文件元信息,包括<level,number,file_size,smallest,largest>
        }
    ...
    void DBImpl::CompactMemTable() { // NOTE:htt, 将immemtable写入sst,并将原有的文件信息写入到mainfest,同时将新文件信息追加到mainfest中
        mutex_.AssertHeld();
        assert(imm_ != NULL);
        ...
        if (s.ok()) {
            ...
            edit.SetLogNumber(logfile_number_);  // Earlier logs no longer needed // NOTE:htt, 设置当前VersionEdit对应的log number
            s = versions_->LogAndApply(&edit, &mutex_); // NOTE:htt, 根据edit和VersionSet生成新的Version,并保存当前的文件信息到mainfest中,并追加新的edit内容到mainfest
        }
    ...
    void DBImpl::CompactRange(const Slice* begin, const Slice* end) { // NOTE:htt, 先尝试对immutable进行合并,在对level[0,6]之间[begin,end]进行manual合并
        ...
        TEST_CompactMemTable(); // TODO(sanjay): Skip if memtable does not overlap // NOTE:htt, 测试对immutable进行compaction,合并到level0层
        for (int level = 0; level < max_level_with_files; level++) {
            TEST_CompactRange(level, begin, end); // NOTE:htt, 尝试[level,begin,end]进行合并
        }
    }

    void DBImpl::TEST_CompactRange(int level, const Slice* begin,const Slice* end) { // NOTE:htt, 尝试[level,begin,end]进行合并
        assert(level >= 0);
    ...
    void DBImpl::MaybeScheduleCompaction() { // NOTE:htt, 尝试进行后台段合并操作
        ...
        if (bg_compaction_scheduled_) {
            ...
            // Already got an error; no more changes
                ...
                manual_compaction_ == NULL &&
                !versions_->NeedsCompaction()) { // NOTE:htt, 判断是否需要compact,compaction_score_>=1 或file_to_compact_有值则进行
            // No work to be done
        } else {
    ...
    void DBImpl::BackgroundCall() { // NOTE:htt, 后台执行文件的合并,并生成sst文件,最后会生成新的Version,并写入到mainfest,并清理无用文件
        MutexLock l(&mutex_);
        ...
        if (shutting_down_.Acquire_Load()) { // NOTE:htt, 关闭则停止
            ...
            BackgroundCompaction(); // NOTE:htt, 后台执行文件的合并,并生成sst文件,最后会生成新的Version,并写入到mainfest,并清理无用文件
        }
    ...
    void DBImpl::BackgroundCompaction() { // NOTE:htt, 后台执行文件的合并,并生成sst文件,最后会生成新的Version,并写入到mainfest,并清理无用文件
        mutex_.AssertHeld();

        if (imm_ != NULL) { // NOTE:htt, 如immemtable不为null,则执行immemtable的compaction,即刷盘
            CompactMemTable(); // NOTE:htt, 将immemtable写入sst,并将原有的文件信息写入到mainfest,同时将新文件信息追加到mainfest中
            return;
        }
        ...
        if (is_manual) { // NOTE:htt, 确认人工合并下待合并的文件列表(level和level+1层)
            ...
            c = versions_->CompactRange(m->level, m->begin, m->end); // NOTE:htt, 选择level层和[begin,end]有交集文件列表作为level层待compact文件列表,并确认level+1文件列表
            m->done = (c == NULL);
            if (c != NULL) {
            ...
            c = versions_->PickCompaction();// NOTE:htt, 选择待compact文件,并扩容level层文件列表(前提是扩容后总文件之后小于50M)
        }
        ...
        if (c == NULL) {
            ...
            c->edit()->AddFile(c->level() + 1, f->number, f->file_size,
                    f->smallest, f->largest); // NOTE:htt, level+1层加入对应文件
            status = versions_->LogAndApply(c->edit(), &mutex_); // NOTE:htt, 根据edit和VersionSet生成新的Version,并保存当前的文件信息到mainfest中,并追加新的edit内容到mainfest
            if (!status.ok()) {
            ...
            status = DoCompactionWork(compact); // NOTE:htt, 执行文件的合并,并生成sst文件,最后会生成新的Version,并写入到mainfest
            if (!status.ok()) {
                RecordBackgroundError(status);
            }
    ...
    Status DBImpl::OpenCompactionOutputFile(CompactionState* compact) { // NOTE:htt, 打开待compact的文件
        ...
        if (s.ok()) {
            compact->builder = new TableBuilder(options_, compact->outfile); // NOTE:htt, 完成整个sstable写入,包括{data block列表, meta block, meta index block, index block, footer} 写入
        }
        return s;
    }
    ...
    Status DBImpl::FinishCompactionOutputFile(CompactionState* compact,
        ...
        if (s.ok()) {
            s = compact->builder->Finish(); // NOTE:htt, 完成整个sstable写入, 包括{data block列表, meta block, meta index block, index block, footer} 写入
        } else {
            compact->builder->Abandon();
        }
    ...
    Status DBImpl::InstallCompactionResults(CompactionState* compact) { // NOTE:htt, 记录本次待删除以及待添加的文件(compact之后文件),并生成新的Version和保存到mainfest
        mutex_.AssertHeld();
        ...
        compact->compaction->AddInputDeletions(compact->compaction->edit()); // NOTE:htt, 将本次compaction涉及到 level/level+1 层文件添加到待删除列表
        const int level = compact->compaction->level();
        for (size_t i = 0; i < compact->outputs.size(); i++) {
            const CompactionState::Output& out = compact->outputs[i];
            compact->compaction->edit()->AddFile(
                    level + 1,
                    out.number, out.file_size, out.smallest, out.largest); // NOTE:htt, level+1层增加本次compaction文件
        }
        return versions_->LogAndApply(compact->compaction->edit(), &mutex_); // NOTE:htt, 根据edit和VersionSet生成新的Version,并保存当前的文件信息到mainfest中,并追加新的edit内容到mainfest
    }

    Status DBImpl::DoCompactionWork(CompactionState* compact) { // NOTE:htt, 执行文件的合并,并生成sst文件,最后会生成新的Version,并写入到mainfest
        ...
        assert(compact->outfile == NULL);
        if (snapshots_.empty()) {
            compact->smallest_snapshot = versions_->LastSequence(); // NOTE:htt, 空快照情况下,当前最小的snapshot即当前versions.seq
        } else {
        ...
        Iterator* input = versions_->MakeInputIterator(compact->compaction); // NOTE:htt, 构建compaction中涉及两层需要merge文件的合并的迭代器
        input->SeekToFirst();
        Status status;
        ...
        for (; input->Valid() && !shutting_down_.Acquire_Load(); ) { // NOTE:htt, MegerIterator 指向下一个值,并重新从所有待compact文件中找到值最小值的位置
            ...
            if (!ParseInternalKey(key, &ikey)) {
                ...
                if (last_sequence_for_key <= compact->smallest_snapshot) { // NOTE:htt, 比compact最小的key小则丢弃
                    ...
                    drop = true;    // (A)
                        ...
                        compact->compaction->IsBaseLevelForKey(ikey.user_key)) { // NOTE:htt, 更高层没有该记录,并为删除标记,同时比compaction的sequence小,则可以删
                    // For this user key:
        ...
        stats.micros = env_->NowMicros() - start_micros - imm_micros; // NOTE:htt, compaction消耗时间,imm落0层统计已专门处理,此处不重复统计
        ...
        if (status.ok()) {
            status = InstallCompactionResults(compact); // NOTE:htt, 记录本次待删除以及待添加的文件(compact之后文件),并生成新的Version和保存到mainfest
        }
        if (!status.ok()) {
        ...
        VersionSet::LevelSummaryStorage tmp;
        Log(options_.info_log,
                "compacted to: %s", versions_->LevelSummary(&tmp)); // NOTE:htt, 统计所有level层文件个数
        return status;
    }
    ...
    namespace {
        ...
        static void CleanupIteratorState(void* arg1, void* arg2) { // NOTE:htt, 清理IterState,包括减少mem/version引用,并删除IterState
    ...
    int64_t DBImpl::TEST_MaxNextLevelOverlappingBytes() {// NOTE:htt, 获取所有从level1层开始每个文件和下一层文件交集,并获取交集最大的值
        MutexLock l(&mutex_);
        return versions_->MaxNextLevelOverlappingBytes();// NOTE:htt, 获取所有从level1层开始每个文件和下一层文件交集,并获取交集最大的值
    }

    Status DBImpl::Get(const ReadOptions& options,
            const Slice& key,
            std::string* value) {/*{{{*/ // NOTE:htt, 读取key对应value,其中先找mem,再找imm,最后从sst找,从sst查找到会尝试触发读合并
        Status s;
        MutexLock l(&mutex_);
        ...
        // Unlock while reading from files and memtables
        {
            ...
            if (mem->Get(lkey, value, &s)) { // NOTE:htt, 如果mem中找到, 为KeyComparator(InternalKeyComparator(BytewiseComparatorImpl)),先比较user_key(按递增序),如果相等则按seq递减排序:{key1,10,1},{key1,8,1},{key2,11,1},用户查询时seq为最新({key1,20,1},则比{key1,10,1}小,则会返回{key1,10,1})
                // Done
            } else if (imm != NULL && imm->Get(lkey, value, &s)) {
                // Done
            } else {
                s = current->Get(options, lkey, value, &stats); // NOTE:htt, 从levelDB的7层文件来读取数据,先从0层读取,如果未找到则继续1层读取
                have_stat_update = true;
            }
            mutex_.Lock();
        }

        if (have_stat_update && current->UpdateStats(stats)) { // NOTE:htt, 根据空读的次数来减少allowed_seeks,如果为0则记录文件为待compaction, 即因读引发的compaction
            MaybeScheduleCompaction(); // NOTE:htt, 尝试进行后台段合并操作
        }
        mem->Unref();
    ...
    Iterator* DBImpl::NewIterator(const ReadOptions& options) { // NOTE:htt, 根据{mem,imm, level0-6}文件迭代器构建DBIter
        ...
        Iterator* iter = NewInternalIterator(options, &latest_snapshot, &seed); // NOTE:htt, 生成{mem, imm, level0-6}文件迭代器,并注册清理函数;
    ...
    // Convenience methods
    Status DBImpl::Put(const WriteOptions& o, const Slice& key, const Slice& val) {// NOTE:htt, 取批量writer记录,并先写WAL,然后写Memtable
        return DB::Put(o, key, val);// NOTE:htt, 取批量writer记录,并先写WAL,然后写Memtable
    }

    Status DBImpl::Delete(const WriteOptions& options, const Slice& key) {// NOTE:htt, 取批量writer记录,并先写WAL,然后写Memtable
        return DB::Delete(options, key);
    }

    Status DBImpl::Write(const WriteOptions& options, WriteBatch* my_batch) { // NOTE:htt, 取批量writer记录,并先写WAL,然后写Memtable
        Writer w(&mutex_);
        w.batch = my_batch;
        ...
        Status status = MakeRoomForWrite(my_batch == NULL);// NOTE:htt, 为写腾空间,主要是将memtable转变为imm,然后生成新的mem来支持写
        uint64_t last_sequence = versions_->LastSequence();
        Writer* last_writer = &w; // NOTE:htt, 此时已经是尾部并且未完成
        if (status.ok() && my_batch != NULL) {  // NULL batch is for compactions
            WriteBatch* updates = BuildBatchGroup(&last_writer); // NOTE:htt, 将writers_中多个 writer请求追加一起直到大小超过限制,这样可以写入更多记录
            WriteBatchInternal::SetSequence(updates, last_sequence + 1); // NOTE:htt, 将最新的 sequence 写入到批量请求中,写入时会递增sequence
    ...
    WriteBatch* DBImpl::BuildBatchGroup(Writer** last_writer) { // NOTE:htt, 将writers_中多个 writer请求追加一起直到大小超过限制,这样可以写入更多记录
        ...
        while (true) {
            if (!bg_error_.ok()) {
                ...
                break;
                    ...
                    allow_delay &&
                    versions_->NumLevelFiles(0) >= config::kL0_SlowdownWritesTrigger) { // NOTE:htt, 0层sst达到8个,减缓写入,会sleep
                ...
                mutex_.Lock();
                    ...
                    (mem_->ApproximateMemoryUsage() <= options_.write_buffer_size)) { // NOTE:htt, 如果内存小于4M并不强制则完成
                ...
                s = env_->NewWritableFile(LogFileName(dbname_, new_log_number), &lfile); // NOTE:htt, 生成WAL ${name}/${number}.log
                if (!s.ok()) {
    ...
    bool DBImpl::GetProperty(const Slice& property, std::string* value) { // NOTE:htt, 获取统计信息，包括compaction统计或leveldb文件列表统计
        value->clear();
    ...
    void DBImpl::GetApproximateSizes(
        ...
        for (int i = 0; i < n; i++) {
            ...
            InternalKey k1(range[i].start, kMaxSequenceNumber, kValueTypeForSeek); // NOTE:htt, 最小查询intelnalKey{user_key,seq,t}
            InternalKey k2(range[i].limit, kMaxSequenceNumber, kValueTypeForSeek); // NOTE:htt, 最小查询intelnalKey{user_key,seq,t}
    ...
    Status DB::Put(const WriteOptions& opt, const Slice& key, const Slice& value) { // NOTE:htt, 取批量writer记录,并先写WAL,然后写Memtable
        WriteBatch batch;
    ...
    Status DB::Open(const Options& options, const std::string& dbname,
            DB** dbptr) { // NOTE:htt, 打开DB文件,并从CURRENT读取mainfest,然后获取文件信息,并从WAL恢复mem数据;同时产生新的mainfest,并写入CURRENT;最后删除无用文件,并尝试段合并
        *dbptr = NULL;

        DBImpl* impl = new DBImpl(options, dbname);
        impl->mutex_.Lock(); // NOTE:htt,启动时加锁
        VersionEdit edit;
        Status s = impl->Recover(&edit); // Handles create_if_missing, error_if_exists // NOTE:htt, 从CURRENT读取mainfest,获取该快照所有文件,在从WAL日志中恢复数据->memtable->sst文件
        if (s.ok()) {
            ...
            if (s.ok()) {
                ...
                s = impl->versions_->LogAndApply(&edit, &impl->mutex_); // NOTE:htt, 根据edit和VersionSet生成新的Version,并保存当前的文件信息到mainfest中,并追加新的edit内容到mainfest
            }
            if (s.ok()) {
