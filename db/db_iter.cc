...
class DBIter: public Iterator { // NOTE:htt, DB迭代器,可以向前或向后查询
 ...
 private:
  ...
  Random rnd_;
  ssize_t bytes_counter_; // NOTE:htt, [0,2M-1]之间随机值,每次读取<key,value>会减去相应值,如果小于0尝试采样并判断是否需要合并
...
void DBIter::FindNextUserEntry(bool skipping, std::string* skip) { // NOTE:htt, 获取下一个有效user entry,会将valid_设置有效
...
void DBIter::Seek(const Slice& target) { // NOTE:htt, 查找target
  ...
  saved_key_.clear();
  AppendInternalKey(
      &saved_key_, ParsedInternalKey(target, sequence_, kValueTypeForSeek)); // NOTE:htt, 构建internal key
  iter_->Seek(saved_key_);
  if (iter_->Valid()) {
    FindNextUserEntry(false, &saved_key_ /* temporary storage */); // NOTE:htt, 获取下一个有效user entry,会将valid_设置有效
  } else {
    valid_ = false;
  }
}
