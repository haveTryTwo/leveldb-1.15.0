...
int InternalKeyComparator::Compare(const Slice& akey, const Slice& bkey) const { // NOTE:htt,比较内部key,其中用户key升序,{seq,t}降序
  // Order by:
  //    increasing user key (according to user-supplied comparator)
  //    decreasing sequence number
  //    decreasing type (though sequence# should be enough to disambiguate)
  int r = user_comparator_->Compare(ExtractUserKey(akey), ExtractUserKey(bkey)); // NOTE:htt, 先比较用户key, 用户key升序
  if (r == 0) {
...
void InternalKeyComparator::FindShortestSeparator(
      std::string* start,
      const Slice& limit) const { // NOTE:htt,找到比内部key start大,比limit小的最小字符串,如{abcg,seq,t}和{abmn,seq,t},则字符串{abd,MaxSeq,ValueType}
  ...
  if (tmp.size() < user_start.size() &&
    ...
    PutFixed64(&tmp, PackSequenceAndType(kMaxSequenceNumber,kValueTypeForSeek)); // NOTE:htt, {tmp,MaxSeq,ValueType}组合新内部key
...
void InternalKeyComparator::FindShortSuccessor(std::string* key) const { // NOTE: htt,找到比内部key大最小字符串，比如{amn,seq,t},对应字符串为{b,seq,t}
...
bool InternalFilterPolicy::KeyMayMatch(const Slice& key, const Slice& f) const { // NOTE:htt, 采用内部key时转换为用户key进行比较
  return user_policy_->KeyMayMatch(ExtractUserKey(key), f); // NOTE:htt, 抽取用户key进行比较
}

LookupKey::LookupKey(const Slice& user_key, SequenceNumber s) { // NOTE:htt, 通过{user_key, seq, ValueType}构建LookupKey
