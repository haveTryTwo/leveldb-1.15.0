...
class Repairer {
 public:
  ...
  Status Run() { // NOTE:htt, 恢复leveldb文件,先将WAL日志数据写入到ldb/sst中,并构建ldb/sst文件列表信息,再重新生成mainfest-1(并写入${db}/CURRENT)
    ...
    if (status.ok()) {
      ...
      status = WriteDescriptor(); // NOTE:htt, 根据next_file_num/max_seq 以及 ldb/sst文件列表重新生成 mainfest-1文件,并写入到${db}/CURRENT
    }
    if (status.ok()) {
 ...
 private:
  ...
  Status ConvertLogToTable(uint64_t log) { // NOTE:htt, 将WAL日志数据写入 sst中(先写入mem,在写入到sst中)
    ...
    status = BuildTable(dbname_, env_, options_, table_cache_, iter, &meta); // NOTE:htt, 将iter中的数据内容写入到sst文件中(dbname为前缀),生成元信息到meta
    delete iter;
    mem->Unref();
    mem = NULL;
    if (status.ok()) {
  ...
  void ExtractMetaData() { // NOTE:htt, 获取sst/ldb文件元信息
    ...
    for (size_t i = 0; i < table_numbers_.size(); i++) {
      ScanTable(table_numbers_[i]); // NOTE:htt, 扫描ldb/sst文件,获取文件大小,最大值/最小值,以及文件最大的seq,如果文件有异常则尝试修复文件（注意可能会丢数据）
    }
  }
  ...
  Iterator* NewTableIterator(const FileMetaData& meta) { // NOTE:htt, 读取file_number文件,并构建talbe的两层迭代器
    ...
    return table_cache_->NewIterator(r, meta.number, meta.file_size); // NOTE:htt, 读取file_number文件,并构建talbe的两层迭代器
  }

  void ScanTable(uint64_t number) { // NOTE:htt, 扫描ldb/sst文件,获取文件大小,最大值/最小值,以及文件最大的seq,如果文件有异常则尝试修复文件（注意可能会丢数据）
    TableInfo t;
    ...
    if (status.ok()) {
      ...
      RepairTable(fname, t);  // RepairTable archives input file.// NOTE:htt, 读取src文件内容写入到新的copy文件,在重命名为原有src文件
    }
  }

  void RepairTable(const std::string& src, TableInfo t) { // NOTE:htt, 读取src文件内容写入到新的copy文件,在重命名为原有src文件
    ...
    int counter = 0;
    for (iter->SeekToFirst(); iter->Valid(); iter->Next()) { // NOTE:htt, 复制有效内容,注意可能会丢失数据,尤其后续内容读取不成功
  ...
  Status WriteDescriptor() { // NOTE:htt, 根据next_file_num/max_seq 以及 ldb/sst文件列表重新生成 mainfest-1文件,并写入到${db}/CURRENT
    ...
    for (size_t i = 0; i < tables_.size(); i++) {
      ...
      edit_.AddFile(0, t.meta.number, t.meta.file_size,
                    t.meta.smallest, t.meta.largest); // NOTE:htt, 添加所有的ldb文件信息到VersionEdit中
    }
...
Status RepairDB(const std::string& dbname, const Options& options) { // NOTE:htt, 恢复leveldb文件,先将WAL日志数据写入到ldb/sst中,并构建ldb/sst文件列表信息,再重新生成mainfest-1(并写入${db}/CURRENT)
  Repairer repairer(dbname, options);
  return repairer.Run(); // NOTE:htt, 恢复leveldb文件,先将WAL日志数据写入到ldb/sst中,并构建ldb/sst文件列表信息,再重新生成mainfest-1(并写入${db}/CURRENT)
}
