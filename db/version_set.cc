...
static const int64_t kExpandedCompactionByteSizeLimit =
    25 * kTargetFileSize;  // NOTE:htt, compact 扩展文件时最大文件为50M
...
Version::~Version() {
  ...
  for (int level = 0; level < config::kNumLevels;
       level++) {  // NOTE:htt, 遍历当前Version保存的所有level的文件,减少文件引用
...
static bool AfterFile(const Comparator* ucmp, const Slice* user_key,
                      const FileMetaData* f) {  // NOTE:htt, 判断user key是否在文件之后
  // NULL user_key occurs before all keys and is therefore never after *f
  return (user_key != NULL && ucmp->Compare(*user_key, f->largest.user_key()) > 0);
}

static bool BeforeFile(const Comparator* ucmp, const Slice* user_key,
                       const FileMetaData* f) {  // NOTE:htt, 判断user key是否在文件之前
  // NULL user_key occurs after all keys and is therefore never before *f
  return (user_key != NULL && ucmp->Compare(*user_key, f->smallest.user_key()) < 0);
}

bool SomeFileOverlapsRange(
    const InternalKeyComparator& icmp, bool disjoint_sorted_files, const std::vector<FileMetaData*>& files,
  ...
  if (!disjoint_sorted_files) {  // NOTE:htt, 文件相交,则逐个文件判断, level0层会相交,其他层不会相交
    ...
    for (size_t i = 0; i < files.size(); i++) {
      ...
      if (AfterFile(ucmp, smallest_user_key, f) || BeforeFile(ucmp, largest_user_key, f)) {  // NOTE:htt, 没有交集
        // No overlap
      } else {
        return true;  // Overlap
      }
  ...
  if (smallest_user_key !=
      NULL) {  // NOTE:htt, 文件不相交,则先用smallest_user_key找打对应文件,在判断largest_user_key是否在其中
...
Iterator* Version::NewConcatenatingIterator(const ReadOptions& options,
                                            int level)
    const {  // NOTE:htt, 为level N层生成迭代器,先查找文件,然后生成文件的迭代器,包含index迭代器和data迭代器的双层迭代器
  return NewTwoLevelIterator(new LevelFileNumIterator(vset_->icmp_, &files_[level]),  // NOTE:htt, level层文件迭代器
                             &GetFileIterator, vset_->table_cache_,
                             options);  // NOTE:htt, 生成文件迭代器,包含index迭代器和data迭代器的双层迭代器
}

void Version::AddIterators(
    const ReadOptions& options,
    std::vector<Iterator*>* iters) {  // NOTE:htt, 针对level N 每层都生成文件迭代器,以便文件查找;其中level
                                      // 0层是每个文件生成迭代器, >= level 1层,则每层一个迭代器
  // Merge all level zero files together since they may overlap
  for (size_t i = 0; i < files_[0].size(); i++) {  // NOTE:htt, level 0层文件有可能重叠,则每个文件都构建一个迭代器
    iters->push_back(vset_->table_cache_->NewIterator(
        options, files_[0][i]->number,
        files_[0][i]->file_size));  // NOTE:htt, 读取level 0层number文件,构建talbe的两层迭代器
  }

  // For levels > 0, we can use a concatenating iterator that sequentially
  // walks through the non-overlapping files in the level, opening them
  // lazily.
  for (int level = 1; level < config::kNumLevels;
       level++) {  // NOTE:htt, >= level 1层文件无重叠，则可以单层构建一个迭代器
    if (!files_[level].empty()) {
      iters->push_back(NewConcatenatingIterator(
          options, level));  // NOTE:htt, 为level
                             // N层生成迭代器,先查找文件,然后生成文件的迭代器,包含index迭代器和data迭代器的双层迭代器
    }
  }
}
...
static void SaveValue(void* arg, const Slice& ikey,
                      const Slice& v) {  // NOTE:htt, 通过internalKey来确认用户key是否存在,如果存在则保存对应的value
...
void Version::ForEachOverlapping(Slice user_key, Slice internal_key, void* arg,
  ...
  for (uint32_t i = 0; i < files_[0].size();
       i++) {  // NOTE:htt, 遍历 level0层的所有文件,如果 user_key在文件范围内则保存下来
...
Status Version::Get(
    const ReadOptions& options, const LookupKey& k, std::string* value,
  ...
  for (int level = 0; level < config::kNumLevels; level++) {  // NOTE:ht, 如果0没找到则继续1层,如果1层没找到继续2层
      ...
      for (uint32_t i = 0; i < num_files; i++) {
        ...
        if (ucmp->Compare(user_key, f->smallest.user_key()) >= 0 &&
            ucmp->Compare(user_key, f->largest.user_key()) <=
                0) {  // NOTE:htt, 如果user_key在 [f->smallest, f->larget]之间
          tmp.push_back(f);
        }
      }
    ...
    for (uint32_t i = 0; i < num_files; ++i) {  // NOTE:htt, 遍历获取到文件,查找ikey
      if (last_file_read != NULL && stats->seek_file == NULL) {
        ...
        stats->seek_file =
            last_file_read;  // NOTE:htt, 上一次读取的文件,即上一次未找打,如果此次找到,则对上一次文件做合并操作
      ...
      s = vset_->table_cache_->Get(options, f->number, f->file_size, ikey, &saver,
                                   SaveValue);  // NOTE:htt, 查找ikey,并且如果找到则保存value到 saver.value
      if (!s.ok()) {
        return s;
      }
      switch (saver.state) {
...
bool Version::UpdateStats(
    const GetStats& stats) {  // NOTE:htt, 根据空读的次数来减少allowed_seeks,如果为0则记录文件为待compaction
...
bool Version::RecordReadSample(
    Slice internal_key) {  // NOTE:htt, 读采样,保存首次match的文件,如果match超过2个文件则尝试更新待合并文件
  ...
  // finding such files?
  if (state.matches >=
      2) {  // NOTE:htt, 如果match的文件超过2个,则认为第一个文件allowed_seeks需减1,然后可以作为compact文件
...
bool Version::OverlapInLevel(
  ...
  return SomeFileOverlapsRange(vset_->icmp_, (level > 0), files_[level], smallest_user_key,
                               largest_user_key);  // NOTE:htt, user_key 是否和level n层文件是否相交
}

int Version::PickLevelForMemTableOutput(
    const Slice& smallest_user_key,
    const Slice&
        largest_user_key) {  // NOTE:htt, 选择MemTable可以直接落入层,范围level [0,1,2]这三层文件,目标避免0->1的compact
  int level = 0;
  if (!OverlapInLevel(0, &smallest_user_key,
                      &largest_user_key)) {  // NOTE:htt, 如MemTable和level0层没交集,继续尝试从[0,1,2]考虑
    ...
    while (level < config::kMaxMemCompactLevel) {  // NOTE:htt, 考虑[0,1,2]做为和MemTable compact的层
      if (OverlapInLevel(level + 1, &smallest_user_key,
                         &largest_user_key)) {  // NOTE:htt, 如level+1层和user_key有交集,选择level层
        break;
      }
      if (level + 2 < config::kNumLevels) {
        // Check that file does not overlap too many grandparent bytes.
        GetOverlappingInputs(level + 2, &start, &limit,
                             &overlaps);  // NOTE:htt, 获取level+2层和 [begin, end]有交集的文件
        const int64_t sum = TotalFileSize(overlaps);
        if (sum >
            kMaxGrandParentOverlapBytes) {  // NOTE:htt,
                                            // 如果level+2层和[start,limit]交集文件大于20M,则不考虑level2层,选择level层开始
          break;
        }
      }
      level++;
...
void Version::GetOverlappingInputs(
    ...
    std::vector<FileMetaData*>*
        inputs) {  // NOTE:htt, 获取 level 层和 [begin, end]有交集的文件,其中level0层需考虑文件交集范围扩大
  assert(level >= 0);
    ...
    if (begin != NULL && user_cmp->Compare(file_limit, user_begin) < 0) {
      ...
      if (level == 0) {  // NOTE:htt, level 0层文件之间并没有严格有序,需要根据加入的文件重新判定[user_begin, user_end]
        ...
        if (begin != NULL && user_cmp->Compare(file_start, user_begin) < 0) {
          user_begin =
              file_start;  // NOTE:htt, 如果文件start比user_begin小,则说明检查的开始范围要变小,并需要重新计算交集文件
...
class VersionSet::
    Builder {  // NOTE:htt,
               // 根据当前base_->files_[level]和新增文件(levels_[level].added_files)生成新的Version->files_[level]文件
 private:
 ...
 public:
  ...
  void Apply(
      VersionEdit* edit) {  // NOTE:htt, 将VersionEdit中删除和添加的文件加入到各level层文件状态,包括删除和添加文件信息
    ...
    for (VersionEdit::DeletedFileSet::const_iterator iter = del.begin(); iter != del.end();
    ...
    for (size_t i = 0; i < edit->new_files_.size(); i++) {
      ...
      if (f->allowed_seeks < 100)
        f->allowed_seeks = 100;  // NOTE:htt, 在读失效场景,最小seek的空读4M(把16K看出40K),对应compect为160K
  ...
  void SaveTo(Version* v) {  // NOTE:htt, 将当前最新状态保存到新的version中,合并base_ version文件和
                             // edit中新增文件,并去除edit删除文件
    BySmallestKey cmp;
    cmp.internal_comparator = &vset_->icmp_;
    for (int level = 0; level < config::kNumLevels;
         level++) {  // NOTE:htt, 将level层原有文件(base_->files)和新增文件(levels_[level].added_files)归并到新的
                     // version->files_[level]
      ...
      for (FileSet::const_iterator added_iter = added->begin(); added_iter != added->end();
           ++added_iter) {  // NOTE:htt,
                            // 归并levels_[level].added_files和base_->files_[level]中文件,并追加新的v->files_[level]
        ...
        MaybeAddFile(v, level,
                     *added_iter);  // NOTE:htt, 将 levels_[level].added_files中迭代的文件加入到新的v->files_[level]
      }
...
Status VersionSet::LogAndApply(
  ...
  Version* v = new Version(this);
  {
    ...
    builder.SaveTo(
        v);  // NOTE:htt, 将当前最新状态保存到新的version中,合并base_ version文件和 edit中新增文件,并去除edit删除文件
  }
  ...
  if (descriptor_log_ == NULL) {  // NOTE:htt, 首次生成新的mainfest,并将现有文件compact和所有文件信息保存
    ...
    new_manifest_file =
        DescriptorFileName(dbname_, manifest_file_number_);  // NOTE:htt, 描述文件, ${dbname}/MANIFEST-${number}
    edit->SetNextFile(next_file_number_);
    s = env_->NewWritableFile(new_manifest_file, &descriptor_file_);  // NOTE:htt, 生成mainfest文件实际写入的文件描述符
    if (s.ok()) {
      descriptor_log_ = new log::Writer(
          descriptor_file_);  // NOTE:htt,
                              // 封装mainfest描述符,采用WAL日志块方式分内容,一旦创建除非重新加载则当前会一直使用该
                              // mainfest来存储新增的元信息,风险是如果重启则恢复时间会比较长,因为需要处理这个过程中的文件列表
  ...
  {  // NOTE:htt, 将新的文件信息(包括compact和文件信息)写入到mainfest,同时将MANIFEST-${number}值写入${dbname}/CURRENT
    ...
    if (s.ok() && !new_manifest_file.empty()) {
      s = SetCurrentFile(env_, dbname_,
                         manifest_file_number_);  // NOTE:htt, 将MANIFEST-${number}值写入到${dbname}/CURRENT
    }
...
Status
VersionSet::Recover() {  // NOTE:htt, 从${dbname}/CURRENT获取mainfest文件名,从mainfest读取VersionEdit列表恢复对应Version
  ...
  std::string current;
  Status s =
      ReadFileToString(env_, CurrentFileName(dbname_), &current);  // NOTE:htt, 从${dbname}/CURRENT读取mainfest文件名称
  if (!s.ok()) {
    return s;
  }
  ...
  {  // NOTE:htt, 读取mainfest文件中保存的所有Edit信息,用来生成当前的Version(包括所有的文件信息)
    ...
    while (reader.ReadRecord(&record, &scratch) &&
           s.ok()) {  // NOTE:htt, 读取一条完整的日志,Full或<First,Mid,...,Last>组合
      VersionEdit edit;
      ...
      if (s.ok()) {
        builder.Apply(
            &edit);  // NOTE:htt, 将VersionEdit中删除和添加的文件加入到各level层文件状态,包括删除和添加文件信息
      }
  ...
  if (s.ok()) {
    Version* v = new Version(this);
    builder.SaveTo(
        v);  // NOTE:htt, 将当前最新状态保存到新的version中,合并base_version文件和 edit中新增文件,并去除edit删除文件
  ...
  for (int level = 0; level < config::kNumLevels - 1; level++) {
    ...
    if (level == 0) {
      ...
      score = static_cast<double>(level_bytes) /
              MaxBytesForLevel(
                  level);  // NOTE:htt, 每层最大容量,0/1层10M, 2层100M, 3层1G, 4层10G, 5层100G, 6层1T,
                           // level>1层按照该层文件大小之和除以该层对应默认大小之和,如果超过并且越大则越先触发compaction
    }
...
Status VersionSet::WriteSnapshot(
    log::Writer* log) {  // NOTE:htt, 保存当前compact值以及所有文件信息到mainfest(采用WAL日志格式写入)
...
const char* VersionSet::LevelSummary(LevelSummaryStorage* scratch) const {  // NOTE:htt, 统计所有level层文件个数
  ...
  snprintf(scratch->buffer, sizeof(scratch->buffer), "files[ %d %d %d %d %d %d %d ]", int(current_->files_[0].size()),
           int(current_->files_[1].size()), int(current_->files_[2].size()), int(current_->files_[3].size()),
           int(current_->files_[4].size()), int(current_->files_[5].size()), int(current_->files_[6].size()));
  return scratch->buffer;
}

uint64_t VersionSet::ApproximateOffsetOf(Version* v,
                                         const InternalKey& ikey) {  // NOTE:htt, 查找key在leveldb所有文件中近似offset
  uint64_t result = 0;
  for (int level = 0; level < config::kNumLevels; level++) {
    ...
    for (size_t i = 0; i < files.size(); i++) {
      if (icmp_.Compare(files[i]->largest, ikey) <= 0) {  // NOTE:htt, 比ikey小文件都统计
        ...
        if (tableptr != NULL) {
          result += tableptr->ApproximateOffsetOf(
              ikey.Encode());  // NOTE:htt, 找到key所在block偏移,若key不存在,返回metablockindex偏移
        }
        delete iter;
      }
    }
...
int64_t
VersionSet::MaxNextLevelOverlappingBytes() {  // NOTE:htt,
...
Iterator* VersionSet::MakeInputIterator(
    Compaction* c) {  // NOTE:htt, 构建compaction中涉及两层需要merge文件的合并的迭代器
  ReadOptions options;
  ...
  const int space =
      (c->level() == 0 ? c->inputs_[0].size() + 1 : 2);  // NOTE:htt,如果有level0,则需要 level0.size()+1,否则2个iter即够
      ...
      if (c->level() + which == 0) {
        ...
        for (size_t i = 0; i < files.size(); i++) {
          list[num++] = table_cache_->NewIterator(
              options, files[i]->number,
              files[i]->file_size);  // NOTE:htt, level0层直接构建文件迭代器(包括index iter和data iter)
        }
      } else {
...
Compaction*
VersionSet::PickCompaction() {  // NOTE:htt, 选择待compact文件,并扩容level层文件列表(前提是扩容后总文件之后小于50M)
  Compaction* c;
  int level;
  ...
  if (size_compaction) {  // NOTE:htt, 按文件大小compact,查找level需要compact的文件,当前level层继续compact也需要满足条件
    ...
    for (size_t i = 0; i < current_->files_[level].size(); i++) {  // NOTE:htt, 获取level层需要compact的文件
      ...
      if (compact_pointer_[level].empty() || icmp_.Compare(f->largest.Encode(), compact_pointer_[level]) >
                                                 0) {  // NOTE:htt, 选择的文件需要比compact_pointer_大
    ...
    if (c->inputs_[0].empty()) {  // NOTE:htt, 如果input[0]为空,则从level层第一个文件开始
      ...
      c->inputs_[0].push_back(current_->files_[level][0]);  // NOTE:htt, level层重新从第一个文件选择,则会重新计算largest
                                                            // key,并更新compact_pointer_[level]
    }
  ...
  if (level == 0) {  // NOTE:htt, level0层则选择有交集的文件列表
    ...
    current_->GetOverlappingInputs(0, &smallest, &largest,
                                   &c->inputs_[0]);  // NOTE:htt, level0层和其他有交集sst文件则也加入compact
...
void VersionSet::SetupOtherInputs(
    Compaction* c) {  // NOTE:htt,计算input1待合并文件, 并扩展input0文件列表,如果扩展后和input1文件之和小于50M
  const int level = c->level();
  InternalKey smallest, largest;
  GetRange(c->inputs_[0], &smallest, &largest);  // NOTE:htt, 获取inputs文件列表中最小的key和最大的key

  current_->GetOverlappingInputs(level + 1, &smallest, &largest,
                                 &c->inputs_[1]);  // NOTE:htt, 获取level+1层有交集文件列表
  ...
  if (!c->inputs_[1].empty()) {
    ...
    current_->GetOverlappingInputs(level, &all_start, &all_limit,
                                   &expanded0);  // NOTE:htt, 扩大level层和<all_star,all_limit>交集文件
    const int64_t inputs0_size = TotalFileSize(c->inputs_[0]);
    const int64_t inputs1_size = TotalFileSize(c->inputs_[1]);
    const int64_t expanded0_size = TotalFileSize(expanded0);
    if (expanded0.size() > c->inputs_[0].size() &&
        inputs1_size + expanded0_size <
            kExpandedCompactionByteSizeLimit) {  // NOTE:htt,如果level+1和level
                                                 // expend文件小于50M,则扩容待compact文件列表
      InternalKey new_start, new_limit;
      GetRange(expanded0, &new_start, &new_limit);
      std::vector<FileMetaData*> expanded1;
      current_->GetOverlappingInputs(level + 1, &new_start, &new_limit, &expanded1);
      if (expanded1.size() ==
          c->inputs_[1].size()) {  // NOTE:htt, 如扩展level层文件后inputs_[1]文件交集文件没有变化,则调整inputs_[0]
        Log(options_->info_log, "Expanding@%d %d+%d (%ld+%ld bytes) to %d+%d (%ld+%ld bytes)\n", level,
            int(c->inputs_[0].size()), int(c->inputs_[1].size()), long(inputs0_size), long(inputs1_size),
  ...
  if (false) {
    Log(options_->info_log, "Compacting %d '%s' .. '%s'", level, smallest.DebugString().c_str(),
  ...
  compact_pointer_[level] =
      largest.Encode().ToString();             // NOTE:htt, 调整level层待compact最大key {user_key,seq,t}为largest
  c->edit_.SetCompactPointer(level, largest);  // NOTE:htt, 设置edit的levelcent待compact最大key为largest
}

Compaction* VersionSet::CompactRange(
    int level, const InternalKey* begin,
    const InternalKey*
        end) {  // NOTE:htt, 选择level层和[begin,end]有交集文件列表作为level层待compact文件列表,并确认level+1文件列表
  ...
  SetupOtherInputs(
      c);  // NOTE:htt, 计算input1待comppact文件,并扩容inputs0待compact文件列表,如果扩展后和input1文件之和小于50M
  return c;
}
...
bool Compaction::IsTrivialMove() const {  // NOTE:htt, 是否为小合并
  ...
  return (num_input_files(0) == 1 &&  // NOTE:htt, level层合并只有一个文件
          num_input_files(1) == 0 && TotalFileSize(grandparents_) <= kMaxGrandParentOverlapBytes);
}

void Compaction::AddInputDeletions(
    VersionEdit* edit) {  // NOTE:htt, 将本次compaction涉及到 level/level+1 层文件添加到待删除列表
  for (int which = 0; which < 2; which++) {
    for (size_t i = 0; i < inputs_[which].size(); i++) {
      edit->DeleteFile(level_ + which,
                       inputs_[which][i]->number);  // NOTE:htt, 将 <level+0/1, 文件number> 添加到待删除列表
    }
  }
}
...
bool Compaction::ShouldStopBefore(
    const Slice& internal_key) {  // NOTE:htt, 当前是否需要停止,如果和levle+2层太多重叠则停止
