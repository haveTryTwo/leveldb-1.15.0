...
SkipList<Key,Comparator>::NewNode(const Key& key, int height) { // NOTE:htt, 分配包含key的Node,并且层高为height
...
template<typename Key, class Comparator>
bool SkipList<Key,Comparator>::KeyIsAfterNode(const Key& key, Node* n) const { // NOTE:htt, 判断n->key是否比key小
...
const { // NOTE:htt, 查找和当前key大或相等的节点 /*{{{*/
  Node* x = head_;
...
//        p = GetVarint32Ptr(p, p + 5, &len);  // +5: we assume "p" is not corrupted // NOTE:htt, 获取跳表中key的长度
//        std::string next_key(p, len); // NOTE:htt, 获取key
//
...
//        fprintf(stderr, "  key:");
//        p = key;
//        p = GetVarint32Ptr(p, p + 5, &len);  // +5: we assume "p" is not corrupted // NOTE:htt, 获取key的长度
//        for (size_t i = 0; i < len; ++i) {
//            fprintf(stderr, "%02x", (uint8_t)p[i]);
//        }
//        fprintf(stderr, "\n");
...
template<typename Key, class Comparator>
typename SkipList<Key,Comparator>::Node*
SkipList<Key,Comparator>::FindLessThan(const Key& key) const { // NOTE:htt, 找比key小的node /*{{{*/
  Node* x = head_;
  int level = GetMaxHeight() - 1;// NOTE:htt, 获取当前跳表的最高height
  while (true) {
