...
Status WriteBatch::Iterate(
    Handler* handler) const {  // NOTE:htt, 遍历rep中的<key,value>逐条写入Memtable,根据操作类型Put()或Delete()
  Slice input(rep_);
...
void WriteBatchInternal::SetContents(WriteBatch* b,
                                     const Slice& contents) {  // NOTE:htt, 重新设置WriteBatch.rep_为contents,格式为
                                                               // ${seq}${count}[{${type}${key}${value}]...
