...
struct FileMetaData {    // NOTE:htt, 文件元信息
  ...
  int allowed_seeks;     // Seeks allowed until compaction // NOTE:htt,
                         // 运行无效查询次数,用于读触发compaction
  uint64_t number;       // NOTE:htt, file对应的数字,数字越大,文件越新
  uint64_t file_size;    // File size in bytes // NOTE:htt, 文件大小
  InternalKey smallest;  // Smallest internal key served by table // NOTE:htt, 内部key, {user_key,
                         // seq, t}三者组合
  InternalKey largest;   // Largest internal key served by table // NOTE:htt, 内部key, {user_key,
                         // seq, t}三者组合

  FileMetaData() : refs(0), allowed_seeks(1 << 30), file_size(0) {}
};

class VersionEdit {  // NOTE:htt,
                     // 中间状态,记录compaction后增加以及删除文件,并记录下一次compaction的起始位置
 public:
  VersionEdit() { Clear(); }
  ~VersionEdit() {}
 ...
 private:
  ...
  std::vector<std::pair<int, InternalKey> >
      compact_pointers_;          // NOTE:htt, 记录每个层级{level, {key, seq, t}}下一次compaction位置
  ...
  std::vector<std::pair<int, FileMetaData> >
      new_files_;  // NOTE:htt, 每个层级compaction之后新增文件元信息{level, FileMetaData}
};
