...
bool Reader::ReadRecord(Slice* record, std::string* scratch) { // NOTE:htt, 读取一条完整的日志,Full或<First,Mid,...,Last>组合
...
void Reader::ReportDrop(size_t bytes, const Status& reason) { // NOTE:htt, report drop的信息
  if (reporter_ != NULL &&
      end_of_buffer_offset_ - buffer_.size() - bytes >= initial_offset_) { // NOTE:htt, 如果读取内容偏移超过init offset则report
    reporter_->Corruption(bytes, reason); // NOTE:htt, corrpution异常信息
  }
}

unsigned int Reader::ReadPhysicalRecord(Slice* result) { // NOTE:htt, 读取一条满足记录规范的日志内容,如Full或First或Mid或Last
  while (true) {
    if (buffer_.size() < kHeaderSize) { // NOTE:htt, buffer内容小于kHeaderSize,才继续读取
      if (!eof_) {
        // Last read was a full read, so this is a trailer to skip
        buffer_.clear();
        Status status = file_->Read(kBlockSize, &buffer_, backing_store_); // NOTE:htt,读WAL整个block,读取前需按init_offset偏移
