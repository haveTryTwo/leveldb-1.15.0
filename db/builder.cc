...
Status BuildTable(const std::string& dbname, Env* env, const Options& options, TableCache* table_cache, Iterator* iter,
  ...
  if (iter->Valid()) {
    ...
    TableBuilder* builder = new TableBuilder(options, file);  // NOTE:htt, 完成整个sstable写入,包括{data block列表, meta
                                                              // block, meta index block, index block, footer} 写入
    ...
    if (s.ok()) {
      s = builder->Finish();  // NOTE:htt, 完成整个sstable写入, 包括{data block列表, meta block, meta index block, index
                              // block, footer} 写入
      if (s.ok()) {
