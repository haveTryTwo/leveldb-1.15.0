...
static const int kL0_CompactionTrigger = 4;  // NOTE:htt, level0层有4个文件即触发compaction,可以配置化
...
static const SequenceNumber kMaxSequenceNumber =
    ((0x1ull << 56) - 1);  // NOTE:htt,
                           // 序号值最大为2^56-1,这样底部8位给type(0为删除,1为新增)使用,这样即可以使用64位存储
...
inline Slice ExtractUserKey(const Slice& internal_key) {  // NOTE:htt, 从内部key中取出用户key
  ...
  return Slice(internal_key.data(),
               internal_key.size() - 8);  // NOTE:htt, 从内部key中取出用户key, 末尾8个字节为{seq,t}组合
}
...
inline int InternalKeyComparator::Compare(const InternalKey& a,
                                          const InternalKey& b) const {  // NOTE:htt, 内部key比较
...
class LookupKey {  // NOTE:htt, 通过{user_key, seq, ValueType} 构建 LookupKey
 public:
  ...
  }  // NOTE:htt, memtable使用key,即 ${key_len}{user_key,seq,t}
