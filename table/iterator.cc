...
Iterator::Iterator() {
  cleanup_.function = NULL;  // NOTE: htt, cleanup_.参数设置为null，其中
                             // arg1和arg2由于和function绑定未进行赋值(推荐赋值)
  cleanup_.next = NULL;
}
...
void Iterator::RegisterCleanup(CleanupFunction func, void* arg1,
                               void* arg2) {  // NOTE: htt, 注册新的 <func,arg1,arg2> Cleanup处理
  assert(func != NULL);
  Cleanup* c;
  if (cleanup_.function == NULL) {
