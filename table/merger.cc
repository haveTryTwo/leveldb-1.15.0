...
namespace {
class MergingIterator : public Iterator {  // NOTE:htt, 处理一组iterator,然后进行查找,
                                           // 向下一个next()或前一个prev()推进
 public:
  MergingIterator(const Comparator* comparator, Iterator** children, int n)
      : comparator_(comparator), children_(new IteratorWrapper[n]), n_(n), current_(NULL), direction_(kForward) {
    for (int i = 0; i < n; i++) {
  ...
  virtual ~MergingIterator() {
    delete[] children_;  // NOTE:htt,
                         // 内部生成的，需要自动释放，其他的comparator_外部传入，由外部处理
  }
...
void MergingIterator::FindSmallest() {  // NOTE:htt,
                                        // 找到所有IteratorWrapper中最小的key对应的iterator
    ...
    if (child->Valid()) {
      if (smallest == NULL) {
        smallest = child;
                 ...
                 0) {  // NOTE:htt, 判断iteratorWrapper中缓存key大小
        smallest = child;
      }
    }
  }
...
void MergingIterator::FindLargest() {  // NOTE:htt, 找到所有IteratorWrapper中最大的key对应的iterator
  ...
  for (int i = n_ - 1; i >= 0; i--) {
    ...
    if (child->Valid()) {
      if (largest == NULL) {
        largest = child;
      } else if (comparator_->Compare(child->key(), largest->key()) >
                 0) {  // NOTE:htt, 判断iteratorWrapper中缓存key大小
        largest = child;
      }
    }
...
Iterator* NewMergingIterator(const Comparator* cmp, Iterator** list,
                             int n) {  // NOTE:htt, 构建多个iterator的合并处理
  assert(n >= 0);
