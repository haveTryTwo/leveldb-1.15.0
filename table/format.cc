...
void Footer::EncodeTo(std::string* dst) const {  // NOTE: htt, 编码footer到 dst
  ...
  dst->resize(2 * BlockHandle::kMaxEncodedLength);  // Padding // TODO: htt,
                                                    // 这里resize()保证dst原来无数据，否则可能被阶段
  PutFixed32(dst,
             static_cast<uint32_t>(kTableMagicNumber & 0xffffffffu));  // NOTE: htt, 添加魔数低32位
...
Status BaseUnCompress::UnCompress() { return Status::NotSupported("not support base uncompress!"); }
 ...
 * UnCompress Factory
 */
BaseUnCompress* UnCompressFactory::CreateUnCompress(const char* data, char* buf, BlockContents* result, size_t n,
...
Status ReadBlock(RandomAccessFile* file, const ReadOptions& options, const BlockHandle& handle,
  ...
  //        result->heap_allocated = false;
  //        result->cachable = false;  // Do not double-cache // NOTE:htt,
  //        如果是mmap等缓存,则不用对Block进行再次缓存
  //      } else {
