...
struct Table::Rep {
  ...
  BlockHandle metaindex_handle;  // Handle to metaindex_block: saved from footer // NOTE:htt, meta
                                 // index handle信息
  Block* index_block;            // NOTE:htt, index block加载内存
};

Status Table::Open(const Options& options, RandomAccessFile* file, uint64_t size,
  ...
  *table = NULL;
  ...
  Status s = file->Read(size - Footer::kEncodedLength, Footer::kEncodedLength, &footer_input,
                        footer_space);  // NOTE:htt, 读取footer内容
  if (!s.ok()) return s;
  ...
  if (s.ok()) {
    s = ReadBlock(file, ReadOptions(), footer.index_handle(),
                  &contents);  // NOTE:htt, 从BlockHandle索引读取指向<offset_, size_>数据
    if (s.ok()) {
...
void Table::ReadMeta(const Footer& footer) {  // NOTE:htt, 根据meta index读取meta block, 并构建bloomFilter的解析
  ...
  if (!ReadBlock(rep_->file, opt, footer.metaindex_handle(), &contents)
           .ok()) {  // NOTE:htt, meta index读取指向<offset, size>数据
...
void Table::ReadFilter(const Slice& filter_handle_value) {  // NOTE:htt, 构建bloomFilter的解析
  ...
  rep_->filter = new FilterBlockReader(rep_->options.filter_policy,
                                       block.data);  // NOTE:htt, 构建filter block的读取
}
...
Iterator* Table::BlockReader(void* arg, const ReadOptions& options,
  ...
  Cache* block_cache =
      table->rep_->options.block_cache;  // NOTE:htt, 获取Block Cache,在options中指定是否启动block_cache
  Block* block = NULL;
  ...
  if (s.ok()) {
    ...
    if (block_cache != NULL) {
      ...
      Slice key(cache_key_buffer,
                sizeof(cache_key_buffer));  // NOTE:htt, Block缓存的key: cache_id+handle.offset(),
                                            // offset为block偏移
      ...
      if (cache_handle != NULL) {                                            // NOTE:htt, 存储Block的缓存
        ...
        s = ReadBlock(table->rep_->file, options, handle,
                      &contents);  // NOTE:htt, 从文件中读取Block
        if (s.ok()) {
          block = new Block(contents);
          if (contents.cachable && options.fill_cache) {  // NOTE:htt, 需要缓存时才进行Block缓存
            cache_handle =
                block_cache->Insert(key, block, block->size(),
                                    &DeleteCachedBlock);  // NOTE:htt, 插入<key, Block>到缓存中,deleter为删除value
          }
        }
      }
    } else {
      s = ReadBlock(table->rep_->file, options, handle,
                    &contents);  // NOTE:htt, 直接从文件读取Block
      if (s.ok()) {
        block = new Block(contents);
      }
    }
...
Iterator* Table::NewIterator(
    const ReadOptions& options) const {  // NOTE:htt, 构建两层迭代器,先找index迭代器,在构建data迭代器
  return NewTwoLevelIterator(rep_->index_block->NewIterator(rep_->options.comparator),  // NOTE:htt, index block迭代器
                             &Table::BlockReader, const_cast<Table*>(this), options);
}

Status Table::InternalGet(const ReadOptions& options, const Slice& k, void* arg,
                          void (*saver)(void*, const Slice&,
                                        const Slice&)) {  // NOTE:htt,先读取index iter,再获取data block,并查找data
  Status s;
  ...
  if (iiter->Valid()) {
    ...
    if (filter != NULL && handle.DecodeFrom(&handle_value).ok() &&
      ...
      Iterator* block_iter =
          BlockReader(this, options, iiter->value());  // NOTE:htt, 读取Block,根据需要缓存,构建date block iter
...
uint64_t Table::ApproximateOffsetOf(
    const Slice& key) const {  // NOTE:htt, 找到key所在block偏移,若key不存在,返回meta block index偏移
  Iterator* index_iter = rep_->index_block->NewIterator(rep_->options.comparator);  // NOTE:htt, index block迭代器
