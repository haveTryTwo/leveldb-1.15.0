...
void BlockBuilder::Add(const Slice& key, const Slice& value) { // NOTE: htt, 在block中添加<key,value>,并根据需要生成新restart
  ...
  if (counter_ < options_->block_restart_interval) {
    ...
    while ((shared < min_length) && (last_key_piece[shared] == key[shared])) { // NOTE: htt, 找到当前key和已存储key最大shared长度
      shared++;
    }
  } else { // NOTE: htt, 如果达到restart对应records个数，则生成新的restart; 此时shared为0,即key共享长度从restart开始
    // Restart compression
