...
class Status {  // NOTE: htt, 状态码，包括生成状态码，判断状态码
 ...
 private:
  ...
  //    state_[4]    == code
  //    state_[5..]  == message
  const char* state_;  // NOTE: htt, 如果为kOK则state_为NULL; 否则为数组
                       // state_[0..3]为异常消息长度,state_[4]为code, state_[5]为异常消息
...
inline Status::Status(
    const Status& s) {  // NOTE:htt, 深度copy，是否有必要？因为只有错误时才copy，并且内容应该很短可以接受
