  ...
  virtual Status Read(uint64_t offset, size_t n, Slice* result,
    ...
    if (offset + n > length_) {  // NOTE: htt, 超过文件长度
      ...
      *result = Slice(reinterpret_cast<char*>(mmapped_region_) + offset,
                      n);  // NOTE: htt, result直接指向mapping映射地址
    }
    return s;
  }
};
...
class PosixEnv : public Env {  // NOTE: htt, PosixEnv 环境对象,用于创建 顺序/随机/读写文件,
                               // 创建加锁文件,日志文件;调度线程,创建用户线程
 public:
  PosixEnv();
  virtual ~PosixEnv() {
      ...
      if (s.ok()) {
        ...
        if (base != MAP_FAILED) {
          *result = new PosixMmapReadableFile(fname, base, size,
                                              &mmap_limit_);  // NOTE: htt, maping映射文件
        } else {
          s = IOError(fname, errno);
        }
      }
  ...
  virtual Status RenameFile(const std::string& src,
                            const std::string& target) {  // NOTE: htt, 重命名文件
    Status result;
  ...
  virtual Status LockFile(const std::string& fname,
                          FileLock** lock) {  // NOTE: htt, 打开锁文件(文件加锁成功)
    *lock = NULL;
    Status result;
  ...
  virtual Status NewLogger(const std::string& fname,
                           Logger** result) {  // NOTE: htt, 创建日志文件对象
    FILE* f = fopen(fname.c_str(), "w");
    if (f == NULL) {
      *result = NULL;
      return IOError(fname, errno);
...
void PosixEnv::Schedule(void (*function)(void*),
                        void* arg) {              // NOTE: htt, 启动线程,并将<function, arg>放入等待队列中
  PthreadCall("lock", pthread_mutex_lock(&mu_));  // NOTE: htt, 先加锁

  // Start background thread if necessary
  if (!started_bgthread_) {
    started_bgthread_ = true;  // NOTE: htt, 启动一个后台线程
    PthreadCall("create thread", pthread_create(&bgthread_, NULL, &PosixEnv::BGThreadWrapper,
                                                this));  // NOTE: htt, 启动线程
  }
...
void PosixEnv::StartThread(void (*function)(void* arg),
                           void* arg) {  // NOTE: htt, 创建线程执行 (*function)(arg)
  pthread_t t;
  StartThreadState* state = new StartThreadState;
  state->user_function = function;
  state->arg = arg;
  PthreadCall("start thread", pthread_create(&t, NULL, &StartThreadWrapper, state));  // NOTE: htt, 创建线程执行
}
