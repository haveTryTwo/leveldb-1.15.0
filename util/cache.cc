...
Cache::Handle* LRUCache::Lookup(const Slice& key,
                                uint32_t hash) {  // NOTE: htt, 查找key, 如果找到则增加引用并调整LRU位置/*{{{*/
...
Cache::Handle* LRUCache::Insert(
    ...
    void (*deleter)(const Slice& key,
                    void* value)) {  // NOTE: htt, 插入对象，并判断是否LRU，如果是则LRU淘汰旧数据/*{{{*/
  ...
  e->hash = hash;
  e->refs = 2;  // One from LRUCache, one for the returned handle // TODO: htt,
                // 即使容量不足，当前新加入被删除,但因refs不会当时真删
