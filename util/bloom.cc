...
class BloomFilterPolicy : public FilterPolicy {  // NOTE: htt,
                                                 // bloom过滤器策略,包括生成新的bloom值，和检查值在不在bloom中
 private:
 ...
 public:
  ...
  virtual void CreateFilter(const Slice* keys, int n,
                            std::string* dst) const {  // NOTE: htt, 创建一批keys的bloom值，并追加到dst
  ...
  virtual bool KeyMayMatch(const Slice& key,
                           const Slice& bloom_filter) const {  // NOTE: htt, 判断key是否满足bloom过滤器
    ...
    for (size_t j = 0; j < k; j++) {
      ...
      if ((array[bitpos / 8] & (1 << (bitpos % 8))) == 0)
        return false;  // NOTE: htt, bloom只要有一位为false，则返回false
      h += delta;
    }
    return true;
  }
